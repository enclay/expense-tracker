...
async def currency_handle(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(f"Your current default curreny is: {user.currency}", reply_markup=markup)
...
async def change_currency_callback(update: Update, context: CallbackContext):
    ...
    user = User(update.effective_user.id, currency)
    sql_update_user(user)

    await context.bot.delete_message(chat_id=query.message.chat.id, message_id=query.message.message_id)
    await context.bot.send_message(query.message.chat.id, f"Currency changed to {get_currency_symbol(user.currency) * 3}")
